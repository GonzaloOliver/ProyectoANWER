...
export const actions: Actions<State, State> = {
  async GET_POSTS_LIST({ commit }): Promise<void | Error> {
    // Use webpack to search the images directory matching .json files
    const context = await require.context('@/content/images/', false, /\.json$/);
    const posts = await getContent({ context, prefix: 'images' });
